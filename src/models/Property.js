...
const propertySchema = new Schema({
  ...
  propertyType: {
    ...
    required: true,
    enum: [
      'Self Contained', 'Mini Flat', 'Flat/Apartment', 'Bungalow',
      'Detached Duplex', 'Semi-Detached Duplex', 'Terraced Duplex',
      'Mansion', 'Block of Flats', 'Commercial', 'Plot', 'Office', 'Warehouse', 'Serviced Apartment'
    ]
  },
  flatType: {
    type: String,
    enum: ['Studio', '1 Bedroom', '2 Bedroom', '3 Bedroom', '4 Bedroom', '5+ Bedroom'],
    required: function() {
      return ['Flat/Apartment', 'Serviced Apartment', 'Block of Flats'].includes(this.propertyType);
    }
  },
  ...
  transactionType: {
    ...
    required: function() { return this.listingType === 'For Sale'; }
  },

  // Payment plan (detailed installments/milestones)
  paymentPlans: [{
    name: { type: String },
    type: { type: String, enum: ['Deposit', 'Milestone', 'Monthly', 'Balloon'], default: 'Milestone' },
  ...
  rentalDetails: {
    ...
    preferredTenants: { type: String, enum: ['Anyone', 'Family', 'Bachelor', 'Company'] },
    serviceCharge: { amount: { type: Number, min: 0 }, frequency: { type: String, enum: ['Monthly','Quarterly','Yearly'] } },
  ...
  propertyCondition: { type: String, enum: ['New', 'Excellent', 'Good', 'Needs Renovation'], required: true },
  possessionStatus: { type: String, enum: ['Ready to Move', 'Under Construction'], required: true },
  ...
  amenities: [{
    type: String,
    enum: [
      'Swimming Pool', 'Gym', 'Garden', 'Kids Play Area', 'Clubhouse',
      'Security', 'CCTV', 'Gated Community', 'Lift', 'Generator', 'Inverter', 'Borehole',
      'Piped Water', 'Water Treatment', 'Fire Safety', 'Intercom', 'Visitor Parking', 'Street Lights',
      'Fence', 'Gatehouse', 'Commercial Mall Nearby', 'School Nearby'
    ]
  }],

  // Utilities
  utilities: {
    waterSupply: { type: String, enum: ['Borehole', 'Water Corporation', 'Bottled/Delivered', 'Municipal', 'Both'], default: 'Municipal' },
    powerBackup: { type: String, enum: ['Generator', 'Inverter', 'Full', 'Partial', 'None'], default: 'None' },
  ...
  // Media
  images: [{ url: { type: String, required: true }, caption: { type: String }, isPrimary: { type: Boolean, default: false } }],
  ...
  contactPerson: {
    ...
    role: { type: String, enum: ['Owner', 'Agent', 'Builder', 'Realtor'], required: true }
  },
  ...
  legalDocuments: {
    cOfO: { present: { type: Boolean, default: false }, url: { type: String }, verifiedAt: { type: Date } },
    governorsConsent: { present: { type: Boolean, default: false }, url: { type: String }, verifiedAt: { type: Date } },
    surveyPlan: { present: { type: Boolean, default: false }, url: { type: String }, verifiedAt: { type: Date } },
    deedOfAssignment: { present: { type: Boolean, default: false }, url: { type: String }, verifiedAt: { type: Date } },
    excision: { present: { type: Boolean, default: false }, url: { type: String }, verifiedAt: { type: Date } }
  },

  // Extras
  highlights: [String],
  additionalRooms: [{ type: String, enum: ['Servant Room', 'Study Room', 'Pooja Room', 'Store Room', 'Home Theater', 'Terrace'] }],
...
propertySchema.pre('save', function(next) {
  try {
    if (!this.propertyId) {
      this.propertyId = `NGPROP${Date.now()}${Math.floor(Math.random() * 1000)}`;
    }

    // Slug from title + area + city
    if (!this.slug && this.title) {
      const parts = [this.title, (this.address && this.address.area), (this.address && this.address.city)].filter(Boolean).join(' ');
      this.slug = generateSlug(parts) + `-${Math.floor(Math.random() * 9000) + 1000}`;
    }
